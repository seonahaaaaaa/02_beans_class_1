 ...
 확인되었습니다." /></h4>
		...
		<jsp:forward page="02_forwardSecond.jsp"/>
			<% /*
				...
				forward (URL 변화를 볼 수 없다): 웹 브라우저에는 최초에 호출한 URL이 표시되고, 이동한 페이지의 URL 정보는 확인할 수 없습니다.
						  다른 페이지로 이동 되었는지 확인 불가함.
						  현재 실행중인 페이지와 forward에 의해 호출될 페이지는 Request 객체와 Response 객체를 공유함. 
				
				redirect : URL을 지시된 주소로 바꾸고 해당 주소로 이동, 다른 웹 컨테이너에 있는 주소로 이동하며 
							새로운 페이지에서는 Request와 Response객체가 새롭게 생성됩니다.
